...
/**
 * As we are unable to run integration tests against an embedded Postgres DB, the method countNinosAfterDate causes errors
 * in other embedded databases due to the JSONB syntax.
 *
 * This class is a wrapper which delegates most methods to the real AuditEntryJpaRepository, but overrides the
 * countNinosAfterDate method with a stub.
 *
 * When this bean is active - the test has annotation ActiveProfiles("stubAuditRepoCountNinosAfterDate") - an ArchiveRequest
 * with lastArchiveDate the same as NINOS_EXIST_AFTER_LAST_ARCHIVE_DATE will simulate finding a nino after the last archive
 * date to allow testing of that scenario.  Any other date will not.
 */
@Component
@Primary
...
class AuditEntryJpaRepositoryStubCountNinosAfterDate implements AuditEntryJpaRepository {
    ...
    @Override
    public Long countEntriesBetweenDates(LocalDateTime startDate, LocalDateTime endDate, AuditEventType type, String namespace) {
        return realRepository.countEntriesBetweenDates(startDate, endDate, type, namespace);
    }

    @Override
    public List<AuditEntry> getEntriesBetweenDates(LocalDateTime startDate, LocalDateTime endDate, AuditEventType type, String namespace) {
        return realRepository.getEntriesBetweenDates(startDate, endDate, type, namespace);
    }

    @Override
    public List<CountByUser> countEntriesBetweenDatesGroupedByUser(LocalDateTime startDate, LocalDateTime endDate, AuditEventType type, String namespace) {
        return realRepository.countEntriesBetweenDatesGroupedByUser(startDate, endDate, type, namespace);
    }
    ...
    @Override
    public List<AuditEntry> findAuditHistory(LocalDateTime toDate, List<AuditEventType> eventTypes, Pageable pageable) {
    ...
    @Override
    public List<AuditEntry> findEntriesByCorrelationId(String correlationId, List<AuditEventType> eventTypes) {
